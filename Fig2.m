% Control via thresholds over two demes
clearvars; clc; close all; tic;

% Assumptions and notes
% - control to a moving target (servo)
% - single run with EpiFilter estimation
% - 1 deme controlled other open loop
% - coordinated local control so sum is constant

% Directory of some main code and plotting options
thisDir = cd; cd('main'); mainDir = cd;
cd(thisDir); addpath(genpath(mainDir));
% Default plotting options
[grey1, grey2, cmap] = defaultSet(10); fnt = 25;
...
% Take quantiles of epidemic information
alpha = 0.025; quans = [alpha, 0.5, 1-alpha]; % 95% coverage
...
% If using old form of data then run
if exist('Isumsamp', 'var') && exist('Idemesamp', 'var')
    IRsamp = Isumsamp; Itotsamp = Idemesamp;
    clear("Isumsamp", "Idemesamp");
end
...
%% Simplified projection distributions
...
for i = 1:nid
    ...
    xlim([0 4]);
    if i == 1
        title('posteriors P$(X | I_1^{\tau})$', 'FontSize', fnt);
    end
    if i == nid
        xlabel('$X$ at $\tau$', 'FontSize', fnt);
    ...
    if i == 1
        title('growth P($\hat{I}_{\tau+h} > I_{\tau}$)', 'FontSize', fnt);
    end
    if i == nid
